...
import { Search, Users, Eye, Globe, TrendingUp, Clock } from 'lucide-react';
...
export default function Home() {
  ...
  const fetchLeaderboard = async () => {
    // Fetch top 100 users. 
    // Note: To support time-filtering (Hour/Week) perfectly, you need Sub-collections (e.g. stats/daily).
    // For this V1, 'Forever' is exact. 'Hour/Week' uses 'lastLogin' as a proxy for recency sorting.
    ...
    // Client-side sort for Views vs Followers
    if (tab === 'viewed') {
      users.sort((a, b) => (b.stats?.views || 0) - (a.stats?.views || 0));
    }

    // Time Filter Proxy (Sort by lastLogin timestamp descending)
    if (filter !== 'forever') {
      users.sort((a, b) => {
